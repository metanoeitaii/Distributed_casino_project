...
public class ClientHandler extends Thread {
    ...
    public void run() {
        try {
            ...
            else if (entoli.equals(Message.GET_BALANCE)) {
                String playerId = in.readLine();
                Socket workerSocket = new Socket(workerHosts.get(0), workerPorts.get(0));
                ObjectOutputStream workerOut = new ObjectOutputStream(workerSocket.getOutputStream());
                workerOut.writeObject(Message.GET_BALANCE);
                workerOut.writeObject(playerId);
                workerOut.flush();
                ObjectInputStream workerIn = new ObjectInputStream(workerSocket.getInputStream());
                String balance = (String) workerIn.readObject();
                PrintWriter clientout = new PrintWriter(sock2.getOutputStream(), true);
                clientout.println(balance);
                workerSocket.close();
                sock2.close();
            }
